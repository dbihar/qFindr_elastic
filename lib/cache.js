          ...
          this.elasticSearchClient.search('0', '_doc', qryObj)
            .on('data', function(data) {
                ...
                for(var i = 0; i < len; i++)
                {
                  ...
                  // Shortening filename
                  if(jsonArray[i]["_source"].filename.length > 80){
                    jsonArray[i]["_source"].filename = jsonArray[i]["_source"].filename.substring(0,45).concat("...").concat(path_short.substring(jsonArray[i]["_source"].filename.length-40, jsonArray[i]["_source"].filename.length))
                  }

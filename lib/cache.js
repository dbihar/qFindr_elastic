    ...
    return new Promise(resolve => {
      if (fs.existsSync(this.path)) {
        ...
        */
        ...
        {
          query = query.replace(/\(/g, "* AND *").replace(/\)/g, "* AND *").replace(/\//g, "* AND *").replace(/\^/g, "* AND *").replace(/  /g, " ")
        }
        ...
        for (var item in list_q) {
          if(count_quote % 2 == 1){
            ...
            query_new = query_new.concat(list_q[count_quote].replace(/\./g, "* AND *").replace(/\,/g, "* AND *").replace(/\:/g, "* AND *"))
          }
          count_quote ++;
        }
